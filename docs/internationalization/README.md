...
Translations of the micro-copy of the templates can be found in the `translations` folder. There is one file per language. File names follow the pattern `w3c_website_templates_bundle_intl-icu.{lc}.yaml` where `{lc}` is the language code.

If you need to add a snippet of text, make sure you add it to the file for each language.

If you need to add a language, make sure you create the corresponding file and copy and paste the contents of the English translation file in it as a starting point.
...
- the last (optional) variable is an object containing variables to inject into the string. The variable keys must match variables defined in the string. E.g. to interpolate 'Hello {name}', the variables object must contain a 'name' property (e.g. `{'name': 'Bob'}`)